function [pmsh] = projmsh(mesh,proj,kind)
%PROJMSH projection operators for JIGSAW MSH objects.
%   [PMSH] = PROJMSH(MESH,PROJ,KIND) returns the 'projected'
%   mesh object PMSH resulting from the application of the
...
%
    ...
    if (~isstruct(mesh) || ...
        ~isstruct(proj) || ...
        ~ischar  (kind) )
    error('projmsh:incorrectInputClass', ...
...
%----------------------------------- proj. from MESH to MESH
    ...
    %------------------------------- proj. geometry parts
        ...
        error('projmsh:unsupportedProjection', ...
    ...
    %------------------------------- setup proj.'d object
        ...
        error('projmsh:invalidProjectionKind', ...
